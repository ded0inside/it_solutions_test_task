...
def create_frame(text, offset, background_color, font_color, font_size):
    ...
    # font = ImageFont.truetype("Arial", font_size)  # Нужно для дебагга 
...
def create_subtitle_video(text, font_size=FONT_SIZE, font_color=(255, 255, 255),
    ...
    text_speed = round((text_width + IMAGE_WIDTH) / (FPS * 3))  # Скорость смещения (пикселей/кадр), чтобы уложиться в 3с
    print(text_width)
    print(text_speed)
    while True:
